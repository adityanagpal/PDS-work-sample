...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 19 12:30:20 2018
...
with open_xlsb('90-FEB16IMP.xlsb') as wb:
...
with open_xlsb('90_NOV15-DEC15IMP.xlsb') as wb:
    with wb.get_sheet(2) as sheet:
        for row in sheet.rows():
...
with open_xlsb('Delhi_Air_90_JUL17-AUG17IMP.xlsb') as wb:
    with wb.get_sheet(2) as sheet:
        for row in sheet.rows():
...
with open_xlsb('Delhi_Air_90_JUL18-AUG18IMP.xlsb') as wb:
    with wb.get_sheet(2) as sheet:
        for row in sheet.rows():
            df3.append([item.v for item in row])

df3 = pd.DataFrame(df3[1:], columns=df3[0])

cols1 = df.columns.tolist()
cols2 = df1.columns.tolist()
cols3 = df2.columns.tolist()
cols4 = df3.columns.tolist()
...
df1 = df1.rename(columns={'Applicable Duty  INR':'Applicable Duty-INR'})

results = pd.concat([df,df1,df2,df3])
...
results = results[['Date','HS Code','Product Description','Importer Name','Duty Percentage',
           ...
           'CHA Name', 'City', 'Contact Person','E-Mail',
            'Phone', 'Pin', 'Port Of Origin','QTY', 'State', 'UNIT',
...
results.to_csv('outputChapter90(1).csv')  
